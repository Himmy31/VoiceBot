...
class voice(commands.Cog):
    ...
    async def kick(self, ctx, member : discord.Member):
        ...
        voice=c.fetchone()
        if voice is None:
            await ctx.channel.send(f"{ctx.author.mention} Вы не глава данного канала")
        else:
            channelID = voice[0]
            channel = self.bot.get_channel(channelID)
            ...
            await ctx.channel.send(f'{ctx.author.mention} Выгнал {member.name} из собственного канала ❌')
        conn.commit()
        conn.close()
    ...
    async def limit(self, ctx, limit):
        ...
        if voice is None:
            ...
            await ctx.channel.send(f'{ctx.author.mention} Установил лимит на канал '+ '{}!'.format(limit))
    ...
    async def name(self, ctx,*, name):
        ...
        if voice is None:
            ...
            await ctx.channel.send(f'{ctx.author.mention} Изменил названия канала '+ '{}!'.format(name))
            c.execute("SELECT channelName FROM userSettings WHERE userID = ?", (id,))
            voice=c.fetchone()
            if voice is None:
